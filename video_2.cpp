...
void move(int x1, int y1, int x2, int y2, int socket)
{ 
	double distance = sqrt((x2-x1)*(x2-x1) + (y2-y1)*(y2-y1)); // we know the distance in pixels between our robots
	if (distance < 10) 
	{
		// If the distance is smaller then 10 pixels ( Robots are united, move the robot back for 10 pixels and attack again ) // 1 pixel we move in 4ms 
	send_command("b",4*20,socket);
	}
	else 
	{
		// attack 
		send_command("f",4*distance,socket);
	}
}
 ...
 // Connect to the robot 
  ...
  Mat threshold3; 
   ...
   return 1; 
		...
		inRange(HSV, Scalar(0,0,0),Scalar(100,100,100), threshold3); // values to be modified 
...

   {
			...
			trackFilteredObject(x0, y0, threshold3, cameraFeed);
